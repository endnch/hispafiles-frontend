...
const Api = ({ className }) => {
  ...
  return (
    <center className={ className }>
        ...
        <p>Se encuentra en una etapa temprana de desarrollo, así que úsala bajo tu propio riesgo.</p>
        <p>Durante el proceso de almacenamiento de un hilo, { settings.site.title } utiliza las mismas librerías para parsear los hilos.</p>
        <br />
        ...
        <p>(.html es opcional, si no se específica { '{pagina}' }, se obtendrá la página 0)</p>
        <p>Parsea una página de board entera, incluyendo las 3 últimas respuestas de cada hilo y sus respectivos metadatos</p>
        ...
        <p>Postear cosas directamente desde la API no está planificado (y probablemente nunca lo esté) por motivos de seguridad.</p>
        <p>Si pretendes hacer una app que no sea sólo para lurkear, deberás hacer los posts directamente al servidor de Hispachan.</p>
        <p>Quizá mirando el código de Kusaba X se te haga un poco mas sencillo.</p>
        <br />
        <br />
        <p>De momento, no hay documentación formal, pero la API es fácil de entender si miras los datos devueltos por la misma.</p>
        <p>Puedes probar desde el navegador con unos cuantos hilos, si quieres. Firefox tiene un visor de JSON integrado.</p>
    </center>
  )
}
