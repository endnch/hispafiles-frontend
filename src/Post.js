...
const Post = React.forwardRef(({ thread, post, backlinks, activePosts, setActivePosts}, ref) => {
    return <>
        { thread.postId !== post.postId && <PostMeta thread={ thread } post={ post } /> }
        { post.file && <>
            <span className='filesize'>
                <a target='_blank' rel='noopener noreferrer' href={`${settings.site.url}/${post.file.url}`}>{ post.file.url.split('/').reverse()[0] }</a>            
        ...
        <span id={ post.postId } style={{ position: 'relative', bottom: '128px', display: 'block' }} ref={ activePosts[post.postId] ? ref : undefined }></span>
        { thread.postId === post.postId && <PostMeta thread={ thread } post={ post } /> }
