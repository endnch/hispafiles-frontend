...
const Thread = ({ className }) => {
    ...
    useEffect(() => {
        (async () => {           
            const response = await fetch(`${settings.site.url}/api/hispafiles/${board}/res/${threadId}`)
    ...
    return (
        <div id='hispaBox' className={ className }>
            ...
            <div className='replymode'>
                Archivado por última vez { moment(thread.lastUpdate).format('DD/MM/YY HH:mm') } (Hora Local).
            </div>
            <hr />
            <div id={'thread' + thread.postId + thread.board}>
                <Post
                    ...
                    ref={ ref } />
                {
                    thread.replies && thread.replies.map(post => <table key={ post.postId } id={ 'respuesta' + post.postId } className='respuesta post'>
                        <tbody>
                            <tr>
                                <td className='doubledash' >{ '>>' }</td>
                                <td id={ 'reply' + post.postId } hf-id={ post.postId } className={ activePosts[post.postId] ? 'reply highlight' : 'reply' } >
                                    <Post
                                        thread={ thread }
                                        post={ post }
                                        backlinks={ backlinks }
